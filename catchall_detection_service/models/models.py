...
class EmailCatchAllValidation(Base):
    ...
    catchall_checked = Column(Boolean, default=False)
    catchall_reason = Column(Text)
    confidence = Column(String(20), default="high")
    
    # Detailed SMTP test results
    mx_hosts_tested = Column(Text)      # JSON: ["mx1.example.com"]
    test_email_used = Column(String(500))
    smtp_responses = Column(Text)       # JSON: [{"mx_host": "...", "code": 250}]
